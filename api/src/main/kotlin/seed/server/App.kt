...
fun logging(): Filter = Filter { next ->
    { req ->
        try {
            next(req)
        } catch (t: Throwable) {
            logger.error(t) { "An error occurred while handling ${req.method} ${req.uri}" }
            Response(Status.INTERNAL_SERVER_ERROR).body(t.message ?: "unknown error")
        }
    }
}
